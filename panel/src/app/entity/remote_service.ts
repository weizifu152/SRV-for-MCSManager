import { InstanceStreamListener, removeTrail, resolveDaemonAddressWithSRV } from "mcsmanager-common";
...
export default class RemoteService {
  ...
  public async connect(connectOpts?: Partial<SocketOptions & ManagerOptions>) {
    if (connectOpts) this.config.connectOpts = connectOpts;

    // Resolve SRV record if enabled
    let resolvedIp = this.config.ip;
    let resolvedPort = this.config.port;
    const enableSRV = this.config.enableSRV !== false;

    logger.info(`Connecting to daemon: ${this.config.ip}:${this.config.port}, SRV enabled: ${enableSRV}`);

    try {
      const resolved = await resolveDaemonAddressWithSRV(this.config.ip, this.config.port, enableSRV);
      resolvedIp = resolved.host;
      resolvedPort = resolved.port;
      if (resolved.isSRV) {
        logger.info(`SRV record resolved: ${this.config.ip}:${this.config.port} -> ${resolvedIp}:${resolvedPort}`);
      } else {
        logger.info(`Using direct connection: ${resolvedIp}:${resolvedPort}`);
      }
    } catch (error) {
      // SRV resolution failed, use original values
      logger.warn(`SRV resolution failed, using original values: ${this.config.ip}:${this.config.port}`);
    }

    // Start the formal connection to the remote Socket program
    let addr = `ws://${resolvedIp}:${resolvedPort}`;
    if (resolvedIp.indexOf("wss://") === 0 || resolvedIp.indexOf("ws://") === 0) {
      addr = `${resolvedIp}:${resolvedPort}`;
    }
    if (systemConfig?.ssl) {
