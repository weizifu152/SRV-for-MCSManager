...
const openDialog = (data?: ComputedNodeInfo, uuid?: string) => {
  if (data && uuid) {
    ...
    dialog.data = {
      ...
      enableSRV: (data as any).enableSRV ?? true
    };
  } else {
    ...
    try {
      ...
      if (editMode.value) {
        await updateNode(dialog.uuid, {
          ...dialog.data,
          enableSRV: dialog.data.enableSRV,
          setting: {
            ...dialog.data
          }
        });
    ...
    <a-tabs v-model:activeKey="activeTabKey">
      ...
      <a-tab-pane key="basic" :tab="t('TXT_CODE_cc7b54b9')">
        <a-form ref="formRef" :model="dialog.data" layout="vertical">
          ...
          <a-form-item :label="t('TXT_CODE_enableSRV')" name="enableSRV">
            <a-typography-paragraph>
              <a-typography-text type="secondary">
                {{ t("TXT_CODE_enableSRVInfo") }}
              </a-typography-text>
            </a-typography-paragraph>
            <a-switch v-model:checked="dialog.data.enableSRV" />
          </a-form-item>
        </a-form>
      </a-tab-pane>
