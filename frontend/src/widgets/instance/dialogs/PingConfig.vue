...
<template>
  ...
  >
    <a-form v-if="options" layout="vertical">
      ...
      <a-form-item>
        <a-typography-title :level="5">{{ t("TXT_CODE_enableSRV") }}</a-typography-title>
        <a-typography-paragraph>
          <a-typography-text type="secondary">
            {{ t("TXT_CODE_enableSRVInfoMinecraft") }}
          </a-typography-text>
        </a-typography-paragraph>
        <a-switch
          v-model:checked="options.config.pingConfig.enableSRV"
          :checked-value="true"
